...
// ---------------------------- start Lieux   ----------------------------
    ...
    public String allLieux(Model model, @PathVariable("pageNumber") int thisPage, @RequestParam(defaultValue="codeInsee") String sortField, @RequestParam(defaultValue="asc") String sortDirection)   {
        try {
            Sort sort = Sort.by(sortField);
            sort=  sortDirection.equals("asc") ? sort.ascending() : sort.descending();
            Page<Lieu> page = appService.getAllLieux(thisPage, 12, sort);
    ...
    public String allLieuxCard(Model model, @PathVariable("pageNumber") int thisPage)   {
        try {
            Sort sort = Sort.by("codeInsee").ascending();
            Page<Lieu> page = appService.getAllLieux(thisPage, 9, sort);
            List<Lieu> lieux = page.getContent();
            long totalParts = page.getTotalElements();
            int totalPages = page.getTotalPages();
